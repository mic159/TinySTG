...
void Game::Initialize(int stage, int difficulty)
{
	...
	for(int i = 0; i < SPAWN_MAX; ++ i)
	{
		...
		this->spawn_info[i].interval_count = 60 + i * 30;
	...
	this->score = 0;
	this->highscore = global.highscore;
	this->enemy_move = 0;
	switch(difficulty)
	{
	case 0:
			...
			for(int j = 0; j < ENEMY_MAX; ++ j)
			{
				...
				if(collision == true)
				{
					...
					if(this->enemy[j]->GetType() >= BOSS1)
					{
						for(int k = 0; k < 8; ++ k)
						{
							SetExplode(enemy_x, enemy_y, (rand() % 5 - 2) << 10, (rand() % 5 - 2) << 10);
						}
						DeleteEnemy();
						this->boss = false;
						this->boss_appear_count = BOSS_APPEAR_COUNT;
...
//			dms::Report("bullet_rate = %d", this->bullet_rate);
	...
	if(this->boss == false)
	{
		...
		{
			SetEnemy((5 + rand() % 86) << 10, -10 << 10, ENEMY4, POWER_UP);
			this->item_interval = ITEM_INTERVAL;
		}
	}
	this->bg_y -= (1 << 5);
	...
	if((this->gameover == false) && (this->continue_count > 0))
	{
		...
		if(this->continue_interval <= 0)
		{
			...
			{
				this->gameover = true;
				this->gameover_count = 60 * 5;
			}
			this->continue_interval = 60;
		}
	}
...
void Game::Draw(void)
{
	...
	const unsigned char* buffer = reinterpret_cast<const unsigned char*>(image_data->buffer) + ((this->bg_y >> 10) * 96);
	...
	{
		screen.DrawSprite(image_data->buffer, i * 7, 58, image_data->width, image_data->height, 0);
	}
	image_data = GetImageData(BOMBER_SMALL);
	for(int i = 0; i < this->player.GetBomber(); ++ i)
	{
		screen.DrawSprite(image_data->buffer, 90 - i * 7, 58, image_data->width, image_data->height, 0);
	}
	if((this->continue_count == 0) && (this->gameover == false))
	{
		if(this->draw_stage > 0)
		{
			screen.DrawFont(27, 18, "STAGE %d", this->stage);
		}
		if(this->draw_warning > 0)
		{
			screen.DrawFont(24, 8, "WARNING!");
			screen.DrawFont(12, 16, "the enemy is");
			screen.DrawFont(0, 24, "approaching fast");
		}
	}
	if(this->continue_count > 0)
	{
		screen.DrawFont(21, 16, "CONTINUE?");
		screen.DrawFont(45, 26, "%d", this->continue_count);
	}
	if(this->gameover == true)
	{
		screen.DrawFont(21, 16, "GAME OVER");
	}
	...
	screen.DrawNumberFont(64, 0, "%8d", highscore);
}
...
void Game::SetShot(void)
{
	...
	SoundPlayer& sound_player = SoundPlayer::GetInstance();
	sound_player.Play(this->sound_handle[SOUND_GUN]);
}
	...
	for(int i = 0; i < count; ++ i)
	{
		for(int j = 0; j < BULLET_MAX; ++ j)
		{
			if(this->bullet[j] == NULL)
			{
				...
				{
					target_x += ((rand() % 20 - 10) << 10);
					target_y += ((rand() % 20 - 10) << 10);
					speed += (rand() % 200 - 100);
					if(speed < 0)
					{
						speed = 256;
...
bool Game::FireBullet(void)
{
	...
	{
		if((rand() % 1000) < (this->bullet_rate + 50))
		{
			return true;
		}
	}
	if((rand() % 1000) < this->bullet_rate)
	{
		return true;
	}
	return false;
...
{
	return this->bullet_speed + (rand() % this->bullet_speed_add);
}
...
void Game::Spawn(int index)
{
	...
	if(this->spawn_info[index].repeat_count <= 0)
	{
		...
		case 1:
			...
			this->spawn_info[index].spawn_x = -10 << 10;
			this->spawn_info[index].spawn_y = (10 + rand() % 20) << 10;
			this->spawn_info[index].repeat_count = 3 + (rand() & 3);
			this->spawn_info[index].rapid = rand() & 1;
			break;
		case 2:
			this->spawn_info[index].enemy_number = ENEMY1;
			this->spawn_info[index].spawn_x = 101 << 10;
			this->spawn_info[index].spawn_y = (10 + rand() % 20) << 10;
			this->spawn_info[index].repeat_count = 3 + (rand() & 3);
			this->spawn_info[index].rapid = rand() & 1;
			break;
		case 3:
			...
			this->spawn_info[index].spawn_x = (5 + rand() % 86) << 10;
		...
		case 5:
			...
			this->spawn_info[index].spawn_x = (8 + rand() % 80) << 10;
			this->spawn_info[index].spawn_y = -10 << 10;
			this->spawn_info[index].repeat_count = 1 + (rand() % 3);
			this->spawn_info[index].rapid = 0;
			break;
		case 6:
			this->spawn_info[index].enemy_number = ENEMY5;
			this->spawn_info[index].spawn_x = -10 << 10;
			this->spawn_info[index].spawn_y = (5 + rand() % 15) << 10;
			this->spawn_info[index].repeat_count = 1 + (rand() % 3);
			this->spawn_info[index].rapid = rand() & 1;
			break;
		case 7:
...
void Game::SpawnBoss(void)
{
	...
	{
	case 1:
		SetEnemy((96 / 2) << 10, -15 << 10, BOSS1, 0);
		break;
	case 2:
		SetEnemy((96 / 2) << 10, -15 << 10, BOSS2, 0);
		break;
	case 3:
		SetEnemy((96 / 2) << 10, -32 << 10, BOSS3, 0);
		break;
	case 4:
		SetEnemy((96 / 2) << 10, -32 << 10, BOSS4, 0);
		break;
	default:
		SetEnemy((96 / 2) << 10, -15 << 10, BOSS1, 0);
		break;
...
int Game::SpawnNumber(int stage)
{
	int draw_table[4][7] =
	{
		...
		{5, 5, 10, 10, 8, 4, 4}, // stage4 spawn rate
	};
	int max_number = 0;
	...
	int sum = 0;
	int draw_number = rand() % max_number;
	for(int i = 0; i < static_cast<int>(sizeof(draw_table[stage - 1]) / sizeof(int)); ++ i)
	{
...
void Game::Bomber(void)
{
	...
	for(int i = 0; i < 8; ++ i)
	{
		SetExplode(fighter_x, fighter_y, (rand() % 5 - 2) << 10, (rand() % 5 - 2) << 10);
	}
}
