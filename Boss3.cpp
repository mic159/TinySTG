...
void Boss3::Update(void)
{
	...
	case PHASE_MOVE_UP:
		...
		if(this->y < (0 << 10))
		{
			this->vx = (rand() & 1) ? -1 << 7 : 1 << 7;
			this->vy = (rand() & 1) ? -1 << 6 : 1 << 6;
	...
	{
		global.game->SetBulletToFighter(this->x, this->y + (6 << 10), global.game->GetBulletSpeed(), Game::BULLET_TYPE_DIFFUSE, 1 + (rand() & 1));
